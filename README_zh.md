...
我们推出了重大更新，旨在提升您的使用体验：

### 📚 全新知识库功能 (v0.1.0)
MCP-X 现已支持强大的知识库管理功能！
- **知识库管理**: 上传、查看、搜索和删除知识文档（支持 txt、md、json、csv、xml、html 格式）
- **智能搜索**: 按文件名和内容进行全文搜索，支持高亮显示匹配结果
- **对话引用**: 在聊天时一键引用知识库文档，AI 可基于您的知识库内容进行回答
- **文档预览**: 实时预览文档内容，支持文件元数据显示
- **响应式设计**: 完美适配桌面和移动设备
...
- **全新侧边导航**: 新的导航栏让您可以快速访问对话、智能体、知识库、工具、模型和系统设置。
...
- 📚 **知识库管理**：上传、搜索、引用知识文档，让 AI 基于您的资料回答问题
- 🔄 **模型上下文协议**：支持 stdio 和 SSE 模式下的 MCP AI agent 无缝集成
- 🤖 **智能体系统**：丰富的专业 AI 智能体，覆盖各个领域的专业需求
...
1. **添加文档**：点击侧边栏"知识库" → 上传支持的文件格式（txt, md, json, csv, xml, html）
2. **搜索文档**：在知识库页面使用搜索框，支持按文件名和内容搜索  
3. **引用文档**：在聊天时点击知识库按钮（📚）→ 搜索选择文档 → 自动插入引用
4. **管理文档**：预览、删除文档，查看详细信息和元数据
