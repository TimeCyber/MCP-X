...
const McpEditPopup = ({ _type, _config, _mcpName, onDelete, onCancel, onSubmit }: mcpEditPopupProps) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 检查是否有默认key值的ENV项（如：key0、key1、TAVILY_API_KEY等）
      for (const mcp of mcpList) {
        const mcpServers = mcp.mcpServers;
        if (mcpServers.env && Array.isArray(mcpServers.env)) {
          // 检测是否有默认形式的key值（key0、key1、TAVILY_API_KEY等）
          const defaultKeyWithValue = mcpServers.env.find(([key, value]) => {
            if (!key || !value) return false;
            
            // 检查是否有类似key0、key1形式的默认key
            if (/^key\d+$/.test(key.toLowerCase())) return true;
            
            // 检查是否有包含API_KEY、APIKEY等默认key
            if (typeof value === 'string' && (value.toUpperCase().includes('KEY') || value.toUpperCase().includes('APIKEY'))) {
              // 排除明显是用户输入的key名（例如通常包含YOUR或MY等前缀）
              if (typeof value === 'string' && (value.toUpperCase().includes('YOUR_') || value.toUpperCase().includes('MY_'))) {
                return false;
              }
              return true;
            }
          ...
          if (defaultKeyWithValue) {
            showToast({
              message: t("tools.emptyEnvKey", { name: mcp.name || t("tools.unnamed") }),
              type: "error"
            });
            return;
          }
      ...
      if(typeRef.current.includes("json")) {
        ...
        // 检查JSON模式下是否有默认key值的ENV项
        for (const serverName in newMcpServers) {
          const server = newMcpServers[serverName];
          if (server.env) {
            const envEntries = Object.entries(server.env);
            const hasDefaultKey = envEntries.some(([k, v]) => {
              if (!k || !v) return false;
              
              // 检查是否有类似key0、key1形式的默认key
              if (/^key\d+$/.test(k.toLowerCase())) return true;
              
              // 检查是否有包含API_KEY、APIKEY等默认key
              if (typeof v === 'string' && (v.toUpperCase().includes('API_KEY') || v.toUpperCase().includes('APIKEY'))) {
                // 排除明显是用户输入的key名（例如通常包含YOUR或MY等前缀）
                if (typeof v === 'string' && (v.toUpperCase().includes('YOUR_') || v.toUpperCase().includes('MY_'))) {
                  return false;
                }
                return true;
              }
            ...
            if (hasDefaultKey) {
              showToast({
                message: t("tools.emptyEnvKey", { name: serverName || t("tools.unnamed") }),
                type: "error"
              });
              return;
            }
        ...
        <div className="field-content">
          ...
          <div className="field-item">
            <label>
              ...
              {currentMcp.name === "filesystem" && (
                <button 
                  onClick={async () => {
                    try {
                      // 使用window.ipcRenderer调用主进程的dialog API选择目录
                      const result = await window.ipcRenderer.invoke('dialog:selectFolder');
                      if (result && !result.canceled && result.filePaths.length > 0) {
                        // 获取选择的目录路径                      
                        const folderPath = result.filePaths[0];
                        // 添加到args数组
                        handleMcpChange("args", [...(currentMcpServers.args || []), folderPath]);
                      }
                    } catch (error) {
                      console.error('选择目录失败:', error);
                    }
                  }}
                  title={t("tools.selectFolder")}
